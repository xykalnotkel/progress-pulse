...
const createUpdate = z.object({ action: z.literal("create_update"), appSlug: z.string().trim().toLowerCase().min(1).max(80), title: z.string().trim().min(1).max(160), description: z.string().trim().max(5000).optional(), status: z.enum(["planning", "building", "testing", "shipped"]).default("building"), version: z.string().trim().max(40).optional(), media: z.array(z.string().url()).max(12).default([]), isPublished: z.boolean().default(true) });
