...
export default async function OpenGraphImage({ searchParams }: { searchParams?: Promise<{ title?: string; app?: string }> } = {}) {
  const { title, app }: { title?: string; app?: string } = await (searchParams ?? Promise.resolve({}));
  const heading = (title || "Building ideas into momentum.").slice(0, 78);
  const label = (app || "XySpace Blog").slice(0, 40);
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", position: "relative", overflow: "hidden", background: "#0A090E", color: "white", padding: "74px", fontFamily: "Arial, sans-serif" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", marginTop: 116, maxWidth: 650, fontSize: 68, letterSpacing: -4, fontWeight: 700, lineHeight: 0.96 }}><span>{heading}</span></div>
        <div style={{ display: "flex", marginTop: 29, color: "#C9AAFF", fontSize: 24 }}>{label} · Progress note</div>
      </div>
