...
type Profile = { email: string; display_name: string | null; title: string | null; avatar_url: string | null; banner_url: string | null; bio: string | null; links: ProfileLink[]; badge: AuthorBadge | null; status_text: string | null; status_kind: "online" | "building" | "focus" | "away" | "offline"; activity_text: string | null };
...
type AdminSection = "posts" | "comments" | "apps" | "profile" | "team";
...
export default function AdminDashboard({ section }: { section: AdminSection }) {
  ...
  const [profileStatusKind, setProfileStatusKind] = useState<"online" | "building" | "focus" | "away" | "offline">("offline");
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        setProfileBio(p?.bio ?? "");
        setProfileStatus(p?.status_text ?? "");
        setProfileStatusKind(p?.status_kind ?? "offline");
        setProfileActivity(p?.activity_text ?? "");
  ...
  async function saveProfile(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/admin/profile", {
        ...
        body: JSON.stringify({ displayName: profileName, title: profileTitle || null, avatarUrl: profileAvatar || null, bannerUrl: profileBanner || null, bio: profileBio || null, links: profileLinks, statusText: profileStatus || null, statusKind: profileStatusKind, activityText: profileActivity || null }),
      });
  ...
  return (
    <main className="admin-page">
      <header className="admin-top">
        <button type="button" className="admin-menu" onClick={() => setAdminNavOpen((value) => !value)} aria-label="Buka menu admin" aria-expanded={adminNavOpen}>{adminNavOpen ? <X size={18}/> : <Menu size={18}/>}</button>
      ...
      <div className="admin-layout">
        <aside className={`admin-side ${adminNavOpen ? "admin-side-open" : ""}`}>
          <div className="admin-label">CONTROL ROOM</div>
          <Link className={section === "posts" ? "admin-nav-active" : ""} href="/admin/posts"><FilePlus2 size={16} /> Posts <span>{updatesList?.length ?? 0}</span></Link>
          <Link className={section === "comments" ? "admin-nav-active" : ""} href="/admin/comments"><MessageSquareText size={16} /> Komentar <span>{approvedComments?.length ?? 0}</span></Link>
          {isOwner ? <Link className={section === "apps" ? "admin-nav-active" : ""} href="/admin/apps"><Layers3 size={16} /> Aplikasi <span>{apps.length}</span></Link> : null}
          <Link className={section === "profile" ? "admin-nav-active" : ""} href="/admin/profile"><UserRound size={16} /> Profil publik</Link>
          {isOwner ? <Link className={section === "team" ? "admin-nav-active" : ""} href="/admin/team"><Users size={16} /> XyTeam <span>{teamList.length}</span></Link> : null}
          <Link href="/docs/ai"><WandSparkles size={16} /> AI integration <ExternalLink size={13} /></Link>
        ...
        <section className="admin-content">
          ...
          <div className="admin-panels">
            ...
            {section === "apps" && isOwner ? <form className="admin-panel app-panel" id="new-app" onSubmit={createApp}>
              <div className="panel-heading">
          ...
          {section === "comments" ? <section className="admin-panel moderation-panel" id="comments">
            <div className="panel-heading">
          ...
          {section === "profile" ? <section className="admin-panel profile-panel" id="profile">
            <div className="panel-heading">
            ...
            <form className="profile-form" onSubmit={saveProfile}>
              ...
              <label>Bio<input value={profileBio} onChange={(e) => setProfileBio(e.target.value)} maxLength={240} placeholder="Tulis dua-tiga kalimat tentang lo — tampil di profil publik." /></label>
              <div className="form-row"><label>Status<select value={profileStatusKind} onChange={(e) => setProfileStatusKind(e.target.value as typeof profileStatusKind)}><option value="online">Online</option><option value="building">Building</option><option value="focus">Focus</option><option value="away">Away</option><option value="offline">Offline</option></select></label><label>Label status<input value={profileStatus} onChange={(e) => setProfileStatus(e.target.value)} maxLength={80} placeholder="Lagi ngoding" /></label></div>
              <label>Aktivitas sekarang<input value={profileActivity} onChange={(e) => setProfileActivity(e.target.value)} maxLength={120} placeholder="Merapikan XySpace Blog" /></label>
          ...
          {section === "team" && isOwner ? <section className="admin-panel team-panel" id="team">
            <div className="panel-heading">
          ...
          {section === "posts" ? <section className="admin-panel updates-panel" id="updates-list">
              <div className="panel-heading">
                <div><p className="admin-kicker">BLOG POSTS</p><h2>Daftar update</h2></div>
                <span><span className="small-live" /> OWNER + TEAM</span>
              </div>
              {updatesList === null ? (
