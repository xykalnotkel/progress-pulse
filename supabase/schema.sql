...
-- ============================================================
-- Final browser-access lockdown
-- ============================================================
-- Public pages read through server routes using the service role. Browser
-- roles only need direct SELECT access to the non-sensitive apps table.
...
-- ============================================================
-- Durable public-write abuse controls
-- ============================================================
alter table public.likes add column if not exists visitor_hash text;
alter table public.comment_reactions add column if not exists visitor_hash text;
...
declare
  ...
  v_hits integer;
begin
  if p_key_hash !~ '^[a-f0-9]{64}$' then raise exception 'invalid rate-limit key'; end if;
  if p_action !~ '^[a-z0-9_-]{1,40}$' then raise exception 'invalid rate-limit action'; end if;
  ...
  insert into public.api_rate_limits as limits (
    key_hash, action, window_start, hits, expires_at
  ) values (
    p_key_hash,
    p_action,
    v_window_start,
